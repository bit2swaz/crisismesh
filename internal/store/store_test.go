...
func TestMessagePersistence(t *testing.T) {
	...
	// 5. Assert msg1 exists and content matches
	...
	// SQLite stores time with some precision, so we check if it's close enough or equal.
	// GORM usually handles time roundtrip well.
	if !retrievedPeer.LastSeen.Equal(newTime) {
		// If exact equality fails due to monotonic clock or precision, check diff
